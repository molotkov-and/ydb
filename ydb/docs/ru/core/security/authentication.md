...
Данный механизм применим только для пользователей, которые обслуживаются самой {{ ydb-short-name }}, для, так называемых, встроенных пользователей. При выполнении аутентификации посредством внешних сервисов, например через LDAP-сервер, указанный механизм блокировки не применяется. При каждой попытке аутентификации выполняется соответствующий запрос к внешнему сервису аутентификации.
