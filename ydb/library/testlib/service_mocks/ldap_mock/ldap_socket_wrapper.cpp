...
TLdapSocketWrapper::TLdapSocketWrapper(TAtomicSharedPtr<TInetStreamSocket> listenSocket, const TTlsSettings& tlsSettings)
...
void TLdapSocketWrapper::OnAccept() {
    ...
    if (TlsSettings.EnableSecureConnection) {
...
TString TLdapSocketWrapper::GetClientCertSubjectName() const {
    return ClientSubjectName;

}

bool TLdapSocketWrapper::IsSecure() const {
    return TlsSettings.EnableSecureConnection;
}
...
void TLdapSocketWrapper::SetupCerts() {
    NKikimr::TCertAndKey serverCertAndKey =  NKikimr::GenerateSignedCert(TlsSettings.CaCert, NKikimr::TProps::AsServer());
    TTempFileHandle certificateFile;
    certificateFile.Write(serverCertAndKey.Certificate.c_str(), serverCertAndKey.Certificate.size());

    TTempFileHandle privateKeyFile;
    privateKeyFile.Write(serverCertAndKey.PrivateKey.c_str(), serverCertAndKey.PrivateKey.size());
    ...
    caFile.Write(TlsSettings.CaCert.Certificate.c_str(), TlsSettings.CaCert.Certificate.size());
