...
void TVmMetadataTokenProviderHandler::Handle(NHttp::TEvHttpProxy::TEvHttpIncomingResponse::TPtr& ev) {
    ...
    TString token;
    TDuration refreshPeriod = Settings.ErrorRefreshPeriod;
    TDuration tokenExpiresIn = Max(Settings.ErrorRefreshPeriod, Settings.SuccessRefreshPeriod);
    TEvServiceTokenManager::TStatus status {.Code = TEvServiceTokenManager::TStatus::ECode::SUCCESS, .Message = "Ok"};
        ...
        if (response->Status == "200") {
            ...
            if (NJson::ReadJsonTree(response->Body, &JsonConfig, &jsonValue)) {
                ...
                if (auto it = jsonValueMap.find("access_token"); it == jsonValueMap.end()) {
                    ...
                    status = {.Code = TEvServiceTokenManager::TStatus::ECode::ERROR, .Message = "Result doesn't contain access_token"};
                } else if (token = it->second.GetStringSafe(); token.empty()) {
                    BLOG_ERROR("Got empty token. Request: " << requestInfo);
                    status = {.Code = TEvServiceTokenManager::TStatus::ECode::ERROR, .Message = "Got empty token"};
                } else {
                ...
                status = {.Code = TEvServiceTokenManager::TStatus::ECode::ERROR, .Message = "Can not read json"};
            }
        } else {
            BLOG_ERROR("Error refreshing metadata token, status: " << response->Status << ", error: " << response->Message);
            status = {.Code = TEvServiceTokenManager::TStatus::ECode::ERROR, .Message = TString(response->Message)};
        }
    } else {
        BLOG_ERROR("Error refreshing metadata token, error: " << ev->Get()->Error);
        status = {.Code = TEvServiceTokenManager::TStatus::ECode::ERROR, .Message = ev->Get()->Error};
    }
    refreshPeriod = Min(tokenExpiresIn, refreshPeriod);
    Send(Sender, new NServiceTokenManager::TEvPrivate::TEvUpdateToken(ProviderInfo.GetId(), status, token, refreshPeriod));
    PassAway();
}
